...
/**
 * 根据pac4j-config-1.8.9.jar 添加微信、QQ、新浪微博等的配置，要依赖要去除这个包的依赖
 * 
...
public class PropertiesConfigFactory implements ConfigFactory {
	 ...
	 * 微信
	 */
	public static final String WECHAT_ID = "wechat.id";
	public static final String WECHAT_SECRET = "wechat.secret";

	/**
	 * QQ
	 */
	public static final String QQ_ID = "qq.id";
	public static final String QQ_SECRET = "qq.secret";

	/**
	 * 新浪微博
	 */
	public static final String SINA_ID = "sina.id";
	public static final String SINA_SECRET = "sina.secret";
	...
	public Config build() {
		...
		tryCreateWeChatClient(clients);
		tryCreateQQClient(clients);
		tryCreateSinaClient(clients);
	...
	private void tryCreateWeChatClient(final List<Client> clients) {
		final String id = getProperty(WECHAT_ID);
		final String secret = getProperty(WECHAT_SECRET);
		if (CommonHelper.isNotBlank(id) && CommonHelper.isNotBlank(secret)) {
			final WeChatClient wechat = new WeChatClient(id, secret);
			clients.add(wechat);
		}
	}

	private void tryCreateQQClient(final List<Client> clients) {
		final String id = getProperty(QQ_ID);
		final String secret = getProperty(QQ_SECRET);
		if (CommonHelper.isNotBlank(id) && CommonHelper.isNotBlank(secret)) {
			final QQClient qq = new QQClient(id, secret);
			clients.add(qq);
		}
	}

	private void tryCreateSinaClient(final List<Client> clients) {
		final String id = getProperty(SINA_ID);
		final String secret = getProperty(SINA_SECRET);
		if (CommonHelper.isNotBlank(id) && CommonHelper.isNotBlank(secret)) {
		}
	}
	...
	private void tryCreateSaml2Client(final List<Client> clients) {
		for (int i = 0; i <= MAX_NUM_CLIENTS; i++) {
			...
			final String ientityProviderMetadataPath = getProperty(
					SAML_IDENTITY_PROVIDER_METADATA_PATH.concat(i == 0 ? "" : "." + i));
			final String maximumAuthenticationLifetime = getProperty(
					SAML_MAXIMUM_AUTHENTICATION_LIFETIME.concat(i == 0 ? "" : "." + i));
			final String serviceProviderEntityId = getProperty(
					SAML_SERVICE_PROVIDER_ENTITY_ID.concat(i == 0 ? "" : "." + i));
			final String serviceProviderMetadataPath = getProperty(
					SAML_SERVICE_PROVIDER_METADATA_PATH.concat(i == 0 ? "" : "." + i));
	...
	private void tryCreateOidcClient(final List<Client> clients) {
		for (int i = 0; i <= MAX_NUM_CLIENTS; i++) {
			...
			if (CommonHelper.isNotBlank(id) && CommonHelper.isNotBlank(secret)
					&& CommonHelper.isNotBlank(discoveryUri)) {
				...
				final String clientAuthenticationMethod = getProperty(
						OIDC_CLIENT_AUTHENTICATION_METHOD.concat(i == 0 ? "" : "." + i));
				if (CommonHelper.isNotBlank(clientAuthenticationMethod)) {
					oidcClient.setClientAuthenticationMethod(
